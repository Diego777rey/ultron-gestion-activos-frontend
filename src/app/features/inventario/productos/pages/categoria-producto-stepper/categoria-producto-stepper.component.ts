...
export class CategoriaProductoStepperComponent implements OnInit {
  ...
  protected guardarCategoria(): void {
    ...
    const request = existente?.id_categoria_producto
      ? this.categoriaService.update(existente.id_categoria_producto, payload)
    ...
    request.subscribe({
      next: (cat) => {
        ...
        if (cat.id_categoria_producto) {
          this.cargarSubcategorias(cat.id_categoria_producto);
        }
  ...
  protected eliminarSubcategoria(sub: CategoriaProductoOutput): void {
    if (!sub.id_categoria_producto) {
      return;
    }
    this.categoriaService.remove(sub.id_categoria_producto).subscribe({
      next: () => {
        this.subcategorias.update((list) =>
          list.filter((s) => s.id_categoria_producto !== sub.id_categoria_producto)
        );
      },
