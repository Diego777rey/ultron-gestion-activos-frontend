import { ChangeDetectionStrategy, Component, inject, signal } from '@angular/core';
import { GenericListComponent } from '../../../../shared/components/generic-list/generic-list';
import { TableCellDirective } from '../../../../shared/components/data-table/table-cell.directive';
import { ActionMenuComponent, MenuAction } from '../../../../shared/components/action-menu/action-menu';
import { DefaultEmptyPipe } from '../../../../shared/pipes/default-empty.pipe';
import { TableColumn } from '../../../../shared/models/table-column.model';
import { ListToolbarAction } from '../../../../shared/models/list-toolbar-action.model';
import { PageChange } from '../../../../shared/models/pagination.model';
import { AppDialogService } from '../../../../shared/services/app-dialog.service';
import { SectorService } from '../../services/sector.service';
import { SectorOutput } from '../../interfaces/sector.interface';
import { SectorFormComponent } from '../../dialogs/sector-form/sector-form.component';

@Component({
  selector: 'app-sectores-list',
  imports: [
    GenericListComponent,
    TableCellDirective,
    ActionMenuComponent,
    DefaultEmptyPipe,
  ],
  templateUrl: './sectores-list.component.html',
  styleUrl: './sectores-list.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'app-list-view' },
})
export class SectoresListComponent {
  private readonly sectorService = inject(SectorService);
  private readonly dialogService = inject(AppDialogService);

  protected readonly sectores = signal<SectorOutput[]>([]);
  protected readonly loading = signal(false);
  protected readonly error = signal<string | null>(null);
  protected readonly search = signal('');
  protected readonly pageIndex = signal(0);
  protected readonly pageSize = signal(15);
  protected readonly totalElements = signal(0);

  protected readonly columns: TableColumn<SectorOutput>[] = [
    { key: 'nombre', header: 'Nombre', width: '280px' },
    { key: 'descripcion', header: 'Descripción', width: '360px' },
    { key: 'estado', header: 'Estado', width: '120px' },
    { key: 'acciones', header: '...', width: '50px', align: 'center' },
  ];

  protected readonly toolbarActions: ListToolbarAction[] = [
    { id: 'search', label: 'Buscar' },
    { id: 'clear', label: 'Limpiar Filtro' },
    { id: 'add', label: '+ Adicionar' },
  ];

  protected readonly rowActions: MenuAction[] = [
    { id: 'edit', label: 'Editar', icon: 'edit' },
    { id: 'delete', label: 'Eliminar', icon: 'delete', danger: true },
  ];

  constructor() {
    this.load();
  }

  protected load(): void {
    this.loading.set(true);
    this.error.set(null);
    this.sectorService.findPaginated(this.pageIndex(), this.pageSize(), this.search()).subscribe({
      next: (response) => {
        this.sectores.set(response.content);
        this.totalElements.set(response.pageInfo.totalElements);
        this.loading.set(false);
      },
      error: (err: Error) => {
        this.error.set(err.message || 'No se pudo conectar con el servidor');
        this.loading.set(false);
      },
    });
  }

  protected onPageChange(event: PageChange): void {
    this.pageIndex.set(event.pageIndex);
    this.pageSize.set(event.pageSize);
    this.load();
  }

  protected onToolbarAction(actionId: string): void {
    switch (actionId) {
      case 'search':
        this.pageIndex.set(0);
        this.load();
        break;
      case 'clear':
        this.search.set('');
        this.pageIndex.set(0);
        this.load();
        break;
      case 'add':
        this.openNewDialog();
        break;
    }
  }

  protected openNewDialog(): void {
    this.dialogService
      .openForm(SectorFormComponent, {
        title: 'Nuevo Sector',
        subtitle: 'Registrá una ubicación física (depósito, salón de ventas, etc.)',
        maxWidth: '640px',
      })
      .subscribe((saved) => {
        if (saved) {
          this.load();
        }
      });
  }

  protected openEditDialog(sector: SectorOutput): void {
    this.dialogService
      .openForm(SectorFormComponent, {
        title: 'Editar Sector',
        subtitle: 'Modificá los datos del sector',
        maxWidth: '640px',
        inputs: { sector },
      })
      .subscribe((saved) => {
        if (saved) {
          this.load();
        }
      });
  }

  protected onRowAction(actionId: string, sector: SectorOutput): void {
    if (actionId === 'edit') {
      this.openEditDialog(sector);
      return;
    }
    if (actionId === 'delete' && sector.id_sector != null) {
      this.sectorService.remove(sector.id_sector).subscribe({
        next: () => this.load(),
        error: (err: Error) => this.error.set(err.message || 'No se pudo eliminar el sector'),
      });
    }
  }

  protected trackById = (s: SectorOutput): unknown => s.id_sector;
}
